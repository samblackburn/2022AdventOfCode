...
public class Day16
{
    ...
    private static void Simulate(List<string> myGuess, Dictionary<string, Valve> nodes)
    {
        ...
        foreach (var node in myGuess)
        {
            ...
            Console.WriteLine(
                $"t={time} Moved from {prevNode} to {node}. Valves {string.Join(", ", opened)} are open, releasing {releaseRate}/min for {released} total pressure.");
            prevNode = node;
